...
import {
  addActivityListAddOn,
  deleteActivityListAddOn,
  editActivityListAddOn,
} from "../../redux/update";
...
class ExAdd extends Component {
    ...
    this._unsubscribe = this.props.navigation.addListener("focus", () => {
      ...
      if (activityId2 == "light_intensity") {
        ...
        this.setState({
          ...
          activity_list_addon_show: [
            ...activity_list.light_intensity.filter(
              (item) => item.type === "addon"
            ),
            ...activity_list.moderate_intensity.filter(
              (item) => item.type === "addon"
            ),
            ...activity_list.vigorous_intensity.filter(
              (item) => item.type === "addon"
            ),
          ],
          study: "ทั้งหมด",
  ...
  componentDidUpdate(prevProps, prevState) {
    const { intensityFromExArticle, study, isModalConter, missionName } =
      this.state;
    const {
      user,
      activity_list,
      intensityFromExArticleTemplate,
      statusAddActListAddOn,
      statusDeleteActListAddOn,
      statusGetActivityList,
      statusEditActListAddOn,
    } = this.props;

    if (
      prevState.intensityFromExArticle !== intensityFromExArticle &&
      intensityFromExArticle !== null
    ) {
    ...
    if (
      prevProps.statusAddActListAddOn !== statusAddActListAddOn &&
      statusAddActListAddOn === "success"
    ) {
      this.props.getActivityList(user && user.user_id);
    }
    if (
      prevProps.statusDeleteActListAddOn !== statusDeleteActListAddOn &&
    ...
    if (
      prevProps.statusEditActListAddOn !== statusEditActListAddOn &&
    ...
    if (
      prevProps.statusGetActivityList !== statusGetActivityList &&
      statusGetActivityList === "success"
    ) {
      //ใช้เพื่อให้ตอน add, delete รายการเพิ่มหน้าแสดงผลอัพเดทเรียลไทม์
  ...
  addMissionName(e, activity_name, intensity, mess) {
    //ใช้เพิ่มรายการ activity
    const { user } = this.props;
    this.setState({
      statusCreate: e,
      data: true,
  ...
  seveEitMissionName(mess) {
    ...
    this.props.editActivityListAddOn(
      user.user_id,
      activity_id_edit_focus,
      missionName,
      statusViolence
  ...
  nextAddActivity(activity, intensity, type) {
    ...
    if (this.props.route.params) {
      ...
      this.props.navigation.navigate("ExAddActivity", {
        activity: activity,
        intensity: intensity,
        type: type,
        activity_id: activity_id ? activity_id : intensity,
      });
    } else {
      this.props.navigation.navigate("ExAddActivity", {
        activity: activity,
  ...
  listDataViews() {
    const {
      ...
      data,
      activity_list_show,
      intensityFromExArticle,
      confirmActivityDeleted,
      message,
      statusAnimated,
    } = this.state;
    const { activity_list } = this.props;
    ...
    return (
      <View
        ...
        onPress={() => this.toggleModal(isModalVisible)}
      >
        <View style={styles.modalViewConter}>
          ...
          >
            ...
            {intensityFromExArticle === null ? (
              <Text style={[styles.headActivity, {}]}>
                {t("activities_by_intensity")}
              </Text>
            ) : intensityFromExArticle == "light_intensity" ? (
              <Text style={styles.headActivity}>
                {t("activity_based_on_low")}
              </Text>
            ) : intensityFromExArticle == "moderate_intensity" ? (
              <Text style={styles.headActivity}>
                {t("activity_based_on_moderate")}
              </Text>
            ) : (
              <Text style={styles.headActivity}>
                {t("activity_based_on_high")}
              </Text>
            )}
            ...
            {intensityFromExArticle === null ? (
              <TouchableWithoutFeedback onPress={() => this.checkEditView()}>
                <View
                  style={{ width: 60, marginTop: -16, alignItems: "center" }}
                >
                  <Text style={[styles.headEdit, { marginTop: 16 }]}>
                    {t("edit")}
                  </Text>
                </View>
              </TouchableWithoutFeedback>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Text
                        style={
                          study == "ต่ำ" ? styles.sectionActive : styles.section
                        }
                      >
                        {" "}
                        {t("low")}
                  ...
                  >
                    ...
                    >
                      <Text
                        style={
                          study == "สูง" ? styles.sectionActive : styles.section
                        }
                      >
                        {" "}
                        {t("high")}
          ...
          <View style={{ marginTop: 24 }}>
            {data == true ? (
              <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  activity_list_show.map((item, i) => {
                    return (
                      <TouchableWithoutFeedback
                        key={i}
                        onPress={() =>
                          this.nextAddActivity(
                            languages == "th"
                              ? languages == "th"
                                ? item.activity
                                : item.hasOwnProperty("activity_eng")
                                ? item.activity_eng
                                : item.activity
                              : item.hasOwnProperty("activity_eng")
                              ? item.activity_eng
                ...
                <TouchableWithoutFeedback
                  onPress={() => this.setState({ statusCreate: "createView" })}
                >
                  <View style={{ marginBottom: 200 }}>
                    <View style={styles.missionView}>
                      <Image
                        style={styles.activityImage}
                        source={require("../../assets/images/activity/frame13811.png")}
                      />
  ...
  createView() {
    const { isModalVisible, statusViolence, missionName, statusAnimated } =
      this.state;
    ...
    return (
      <>
        {statusAnimated === true && (
          <Animated.View
            style={{ paddingHorizontal: 16, opacity, transform: [{ scale }] }}
          >
            <View style={styles.activityindicator}></View>
          </Animated.View>
        )}
          ...
          >
            <View style={[styles.modalViewConter, { paddingHorizontal: 16 }]}>
              ...
              >
                <TouchableWithoutFeedback
                  onPress={() =>
                    this.setState({ statusCreate: "listDataViews" })
                  }
                >
              ...
              <Text style={[styles.headActivity, { marginTop: 19 }]}>
                {t("concentration")}
              </Text>
              {/*   <TouchableWithoutFeedback></TouchableWithoutFeedback> */}
              <View style={[styles.missionView, { marginTop: 8 }]}>
                <TouchableWithoutFeedback
                  onPress={() => this.violence("light_intensity")}
                >
                  <View
                    style={[
                      styles.boxCreate,
                      statusViolence == "light_intensity"
                        ? { borderWidth: 2, borderColor: colors.persianBlue }
                        : null,
                    ]}
                  >
                    <Image
                ...
                <TouchableWithoutFeedback
                  onPress={() => this.violence("moderate_intensity")}
                >
                  <View
                    style={[
                      styles.boxCreate,
                      { marginLeft: 16 },
                      statusViolence == "moderate_intensity"
                        ? { borderWidth: 2, borderColor: colors.persianBlue }
                        : null,
                    ]}
                  >
                    ...
                    />
                    <Text style={styles.textImageBoxCreate}>
                      {t("moderate")}
                    </Text>
                  </View>
                </TouchableWithoutFeedback>
                <TouchableWithoutFeedback
                  onPress={() => this.violence("vigorous_intensity")}
                >
                  <View
                    style={[
                      styles.boxCreate,
                      { marginLeft: 16 },
                      statusViolence == "vigorous_intensity"
                        ? { borderWidth: 2, borderColor: colors.persianBlue }
                        : null,
                    ]}
                  >
              ...
              <Text style={styles.textAdd}>
                {statusViolence == "light_intensity" &&
                  `${t("less_strenuous_activity")}`}
                {statusViolence == "moderate_intensity" &&
                  `${t("quite_heavy_physical_exertion")}`}
                {statusViolence == "vigorous_intensity" &&
                  `${t("very_strenuous_activity")}`}
              </Text>
              <TextInput
                style={styles.input}
            ...
            <View style={[styles.modalViewConter, { paddingHorizontal: 16 }]}>
              ...
              <View style={{ marginTop: 24 }}>
                <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        >
                          <View style={{ marginRight: 16 }}>
                            <View style={styles.missionView}>
                              ...
                              />
                              <View
                                style={[styles.groupText2, { width: "69%" }]}
                              >
                                <Text style={styles.headText2}>
                                  {languages == "th"
                                    ? languages == "th"
                  ...
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.setState({ statusCreate: "createView" })
                    }
                  >
          ...
          <>
            ...
            >
              <View style={[styles.modalViewConter, { paddingHorizontal: 16 }]}>
                ...
                >
                  <TouchableWithoutFeedback
                    onPress={() => this.setState({ editmission: false })}
                  >
                    <Image
                  ...
                  {statusViolence && missionName ? (
                    <TouchableWithoutFeedback
                      onPress={() =>
                        this.seveEitMissionName("บันทึกกิจกรรมแล้ว")
                      }
                    >
                ...
                <View style={{ justifyContent: "space-between", flex: 1 }}>
                  <View>
                    ...
                    <TextInput
                      style={styles.input}
                      onChangeText={(text) =>
                        this.setState({ missionName: text })
                      }
                      value={missionName}
                  ...
                  {confirmDelete === false ? (
                    <View style={{ alignItems: "center" }}>
                      ...
                      >
                        <Text style={styles.deleteActivity}>
                          {t("delete_this_event")}
                        </Text>
                      </Pressable>
                    </View>
              ...
              >
                <View style={styles.modalView}>
                  ...
                  >
                    ...
                    <TouchableWithoutFeedback
                      onPress={() => this.deleteActivity("ลบกิจกรรมแล้ว")}
                    >
        ...
        <View style={styles.fill}>
          <View style={{ zIndex: 0 }}>
            <Pressable
              ...
              onPress={() => this.toggleModal(isModalVisible)}
            />
            <Modal isVisible={isModalConter} style={{ margin: 0 }}>
              {statusCreate === "listDataViews"
                ? this.listDataViews()
